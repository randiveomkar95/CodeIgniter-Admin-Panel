...
<a href="<?php base_url(); ?>add_blog">
<li class="breadcrumb-item"><input type="submit" style="border-radius:20px;" class="btn btn-primary" value="Add Blog" name="">
</li>
</a>
...
<td><?php echo nl2br($row->auther); ?></td>
<td><?php echo nl2br($row->date); ?></td>
<td><?php echo nl2br($row->content); ?></td>
<!-- <td><?php echo nl2br($row->generated_at); ?></td> -->
 <td>

<a href="<?php base_url(); ?>edit_blog/<?php $uid = $row->id; echo $uid; ?>">
 <i class='fa fa-pencil' style='font-size:25px; color:#4099FF;'></i>
</a>&nbsp;&nbsp;&nbsp;

    <?=anchor("blog/delete_data/".$row->id,"<i class='fa fa-trash' style='font-size:25px; color:red;'></i>",array('onclick' => "return confirm('Do you want to really delete this record??')"))?>
