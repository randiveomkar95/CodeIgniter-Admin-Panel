...
class Gallary extends CI_Controller {
    ...
    function index(){
              $this->load->model("gallary_model");  
           $data["fetch_data"] = $this->gallary_model->fetch_data();  

            $this->load->view('admin/inc/header');
            $this->load->view('admin/inc/sidebar');
 
           $this->load->view('admin/gallary/view_gallary',$data);
        ...
        if($this->input->post('submitForm') && !empty($_FILES['upload_Files']['name'])){
            ...
            if(!empty($uploadData)){
                ...
                $insert = $this->gallary_model->insert($uploadData);
                $statusMsg = $insert?'Files uploaded successfully.':'Some problem occurred, please try again.';
                $this->session->set_flashdata('statusMsg',$statusMsg);
            }
        }
        //Get files data from database
        // $data['gallery'] = $this->gallary_model->getRows();
        //Pass the files data to view

        redirect(base_url() . 'view_gallary'); 
    }

    function add_gallary(){
        $this->load->view('admin/inc/header');
        $this->load->view('admin/inc/sidebar');
        $this->load->view('admin/gallary/add_gallary');
    }
    ...
    public function upload_multiple()
    {
        ...
        if(isset($_FILES['userfile']) && $_FILES['userfile']['error'] != '4') :
            ...
            for($i=0; $i<$count; $i++):
                ...
                $target_path = './uploads/gallary/thumbs/';
      ...
      public function delete_data($id)
            {  
              //echo $id;
               $this->load->model("gallary_model");  
               $this->gallary_model->delete_data($id);  
               redirect(base_url() . "view_gallary");  
            } 
}